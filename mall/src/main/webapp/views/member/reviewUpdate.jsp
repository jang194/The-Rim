...
<script>
	...
	function viewReviewOk()
	{
       ...
       chk.open("get","reviewUpdateOk?pcode="+pcode+"&star="+star+"&content="+content+"&no="+no+"&ppcode="+ppcode+"&color=${color}&size=${size}");
       chk.send();
	}	
...
<body>
<section>
<form method="post" action="reviewUpdateOk" name="rform">
...
<div style="display:flex;justify-content:space-between;height:60px;line-height:60px;">
		<div style="margin-left:30px;line-height:75px;"><img src="/static/main/gumaelogo.png" onclick="location='../main/main'" style="cursor:pointer;"></div><div style="font-weight:900;font-size:20px;">리뷰 수정</div><div style="line-height:70px;margin-right:15px;cursor:pointer;" onclick="window.close()"><img src="/static/main/close.png"></div>
	</div>
...
<div> <!-- 별 이미지 -->
<c:forEach begin="1" end="${rvo.star}" var="i">
<img src="/static/main/rstar.png" class="star" onclick="chgStar(${i-1})">
</c:forEach>
<c:if test="${rvo.star==1}">
<c:forEach begin="1" end="${5-rvo.star}" var="i">
<img src="/static/main/gstar.png" class="star" onclick="chgStar(${i})">
</c:forEach>
</c:if>
<c:if test="${rvo.star==2}">
<c:forEach begin="1" end="${5-rvo.star}" var="i">
<img src="/static/main/gstar.png" class="star" onclick="chgStar(${i+1})">
</c:forEach>
</c:if>
<c:if test="${rvo.star==3}">
<c:forEach begin="1" end="${5-rvo.star}" var="i">
<img src="/static/main/gstar.png" class="star" onclick="chgStar(${i+2})">
</c:forEach>
</c:if>
<c:if test="${rvo.star==4}">
<c:forEach begin="1" end="${5-rvo.star}" var="i">
<img src="/static/main/gstar.png" class="star" onclick="chgStar(${i+3})">
</c:forEach>
</c:if>
</div>
...
<textarea name="content" onkeyup="lenChk(this.value)" style=" width: calc(100% - 30px);box-sizing:border-box;word-break:break-all;resize:none;height:125px;margin-left:15px;margin-top:10px;outline:none;" placeholder="최소 10자 이상 입력해주세요.">${rvo.content}</textarea>
...
<input style="width:49.5%;height:40px;font-weight:bold;color:white;background:black;border:1px solid black;" type="button" value="수정" onclick="viewReviewOk()">
</div>
</form>
</section>
<div id="modal">
	<div id="modal_body">
		<div style="font-weight:900;margin-top:15px;">리뷰 수정 완료! </div>
		<div></div>
		<input type="button" value="확인" style="border-radius:5px;width:180px;height:40px;margin-top:15px;font-weight:bold;color:white;background:black;border:1px solid black;" onclick="reloadPararent()">
	</div>
</div>
