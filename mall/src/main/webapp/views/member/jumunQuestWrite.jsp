...
//		chk.open("get","jumunQuestWriteOk?jumuncode=${gvo.jumuncode}&pcode=${gvo.pcode}&pimg=${pimg}&title="+title+"&content="+content+"&qtype="+qtype);
		...
		if(qtype.length==0)
			...
			{
			location="jumunQuestWriteOk?jumuncode=${gvo.jumuncode}&pcode=${gvo.pcode}&pimg=${pimg}&title="+title+"&content="+content+"&qtype="+qtype;
...
<section>
  <form name="qform">
	...
	<div style="width:70px;white-space:nowrap;word-break:break-all;min-width:70px;">상품</div><div style="color:black;font-weight:900;">${gvo.title}</div>
	</div>
	<div style="display:flex;justify-content:start;margin-top:15px;">
	<div style="width:70px;min-width:70px;">옵션</div>
	<div>
	<div style="color:#959692;">
		컬러: ${gvo.color} / 사이즈: ${gvo.size} / 수량: ${gvo.su}개
	</div>
	</div>
	</div>
	</div>
	...
	<div style="display:flex;justify-content:start;margin-top:90px;padding-left:15px;font-size:13px;">
	<div>문의유형</div>
	...
	<textarea name="title" style="width:100%;height:50px;border:none;"></textarea>
	</div>
	</div>
	</div>
	...
	<textarea name="content" style="width:100%;height:60px;border:none;"></textarea>
	</div>
	</div>
	</div>
	...
	<input style="width:50px;height:25px;background:#8a909d;color:white;border:1px solid #656975;border-radius:3px;" type="button" value="확인" onclick="jumunQuestWriteOk()">
